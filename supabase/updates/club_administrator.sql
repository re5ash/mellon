-- Mellon: администратор конкретного молодёжного клуба. Выполните весь файл.
begin;
...
do $install$
begin
 if exists(select 1 from app_private.installed_features where feature='club_administrator_20260912') then
  if not exists(select 1 from app_private.installed_features where feature='club_administrator_20260912' and checksum='ea4b9e0a9b0fc78b067b11c613894246a76867d49a3bae5d3d300fd2324a7f66') then raise exception 'A different club administrator update is already installed'; end if;
  raise notice 'Роль администратора молодёжного клуба уже установлена';
 else
  if to_regprocedure('public.save_person_roles_v2(uuid,text,jsonb,text,uuid,uuid)') is null then raise exception 'Сначала выполните mellon_role_catalog.sql (обновление 016)'; end if;
  execute $club_admin_migration$
...
$club_admin_migration$;
  insert into app_private.installed_features(feature,checksum) values('club_administrator_20260912','ea4b9e0a9b0fc78b067b11c613894246a76867d49a3bae5d3d300fd2324a7f66');
 end if;
end $install$;
commit;
