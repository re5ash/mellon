-- Mellon: экран клуба и управляемые чаты. Выполните весь файл.
begin;
...
do $install$
begin
 if exists(select 1 from app_private.installed_features where feature='club_dashboard_20260912') then
  if not exists(select 1 from app_private.installed_features where feature='club_dashboard_20260912' and checksum='fc8c71b181b16aba771d49688cb9bccb8c78536be65ee7c802ff1269952cfa6e') then raise exception 'A different club dashboard update is already installed';end if;
  raise notice 'Экран молодёжного клуба уже установлен';
 else
  if to_regprocedure('public.club_management_cities()') is null then raise exception 'Сначала выполните mellon_club_navigation.sql (обновление 018)';end if;
  execute $dashboard_migration$
...
$dashboard_migration$;
  insert into app_private.installed_features(feature,checksum)values('club_dashboard_20260912','fc8c71b181b16aba771d49688cb9bccb8c78536be65ee7c802ff1269952cfa6e');
 end if;
end $install$;
commit;
