-- Mellon: existing role catalog and single-choice assignments. Run this complete file once.
begin;
select pg_advisory_xact_lock(120016);
...
do $install$
begin
 if exists(select 1 from app_private.installed_features where feature='dynamic_role_catalog_20260912') then
  if not exists(select 1 from app_private.installed_features where feature='dynamic_role_catalog_20260912' and checksum='7e8f094cf86ec578a8f55817385493e6f22bb7f4a8f7cbe72c9331a3b09ab0be') then raise exception 'Другая версия каталога ролей уже установлена'; end if;
  raise notice 'Каталог ролей уже обновлён';
 else
  if to_regprocedure('public.save_person_roles(uuid,text,jsonb,text,uuid,uuid)') is null then raise exception 'Сначала выполните mellon_role_manager.sql из предыдущего обновления'; end if;
...
$catalog_migration$;
  insert into app_private.installed_features(feature,checksum) values('dynamic_role_catalog_20260912','7e8f094cf86ec578a8f55817385493e6f22bb7f4a8f7cbe72c9331a3b09ab0be');
 end if;
end $install$;
commit;
