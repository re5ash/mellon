-- Mellon: живые чаты, вступление/выход и фотография клуба. Выполните файл целиком.
begin;
...
do $install$
begin
 if exists(select 1 from app_private.installed_features where feature='club_live_20260912') then
  if not exists(select 1 from app_private.installed_features where feature='club_live_20260912' and checksum='0516ec2309c7d3eac613b2fbc56290ce8582bf0f30b043ba757f62f82e019ac9') then raise exception 'Different club live update already installed';end if;
  raise notice 'Обновление клуба уже установлено';
 else
  if to_regprocedure('public.club_dashboard(uuid,jsonb)') is null then raise exception 'Сначала выполните mellon_club_dashboard.sql (019)';end if;
  execute $club_live$
...
$club_live$;
  insert into app_private.installed_features(feature,checksum)values('club_live_20260912','0516ec2309c7d3eac613b2fbc56290ce8582bf0f30b043ba757f62f82e019ac9');
 end if;
end $install$;
commit;
