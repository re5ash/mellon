-- Mellon: данные заявителя и переход в «Роли». Выполните файл целиком.
begin;
select pg_advisory_xact_lock(120010);
do $install$
begin
 if not exists(select 1 from app_private.installed_features where feature='guest_review_20260912') then
  raise exception 'Сначала выполните mellon_guest_review.sql';
 end if;
 if exists(select 1 from app_private.installed_features where feature='notification_people_20260913') then
  if not exists(select 1 from app_private.installed_features where feature='notification_people_20260913' and checksum='6c63b6eb1266273634591c5b6c2cd3cd1330ca577686b30c777faec909cbc423') then
   raise exception 'Different notification people update already installed';
  end if;
 else
...
$body$;
  insert into app_private.installed_features(feature,checksum) values('notification_people_20260913','6c63b6eb1266273634591c5b6c2cd3cd1330ca577686b30c777faec909cbc423');
 end if;
end $install$;
commit;
