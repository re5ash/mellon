...
try{
 ...
 for(const f of (await readdir(path.join(root,'supabase/migrations'))).filter(f=>f.endsWith('.sql')&&f>='202609120016').sort())await db.exec(await readFile(path.join(root,'supabase/migrations',f),'utf8'));
 ...
 const details=user=>val('select public.role_manager_person($1)',[user]);
 const save=async(user,global,clubs,options={})=>{
   const revision=options.revision??(await details(user)).revision;
   return val('select public.save_person_roles($1,$2,$3,$4,$5,$6)',[user,global,clubs,revision,options.request??id(req++),options.actor??actor]);
 };
 ...
 await denied('select public.role_manager_person($1)',[peer]);
 ...
 await denied('select public.role_manager_person($1)',[foreign]);
 });
 ...
 for(const u of [member,moderator,null]) { await login(u);await denied('select * from public.role_manager_people()');await denied('select public.role_manager_person($1)',[leader]); }
 });
 await login(superuser);
