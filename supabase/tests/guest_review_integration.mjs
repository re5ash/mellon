import {checkNotificationPeople} from './notification_people_checks.mjs';
...
try{
 ...
 await db.exec('reset role');
 const beforeConfirm=id(9020);
 await q('insert into auth.users(id,email)values($1,$2)',[beforeConfirm,'pending-before@example.test']);
 const reviewSql=await readFile(path.join(root,'supabase/updates/guest_review.sql'),'utf8');
 const assignmentsBefore=(await q('select * from app_private.role_assignments order by id')).rows;
 await check('guest review migration applies twice and preserves all existing accounts/roles',async()=>{
  await db.exec(reviewSql);await db.exec(reviewSql);
  assert.deepEqual((await q('select * from app_private.role_assignments order by id')).rows,assignmentsBefore);
  assert.deepEqual((await q('select * from app_private.roles order by key')).rows,snapshot);
  assert.equal(await val("select count(*)::int from app_private.account_access where review_status is not null"),1);
  assert.equal(await val('select review_status from app_private.account_access where user_id=$1',[beforeConfirm]),'awaiting_email');
 });
 const fresh=id(9001),rejectUser=id(9002),unassigned=id(9003),explicit=id(9004),restricted=id(9005);
 const signup=async(u,{confirmed=false,youth=null}={})=>{
   await db.exec('reset role');
   const metadata={club_registration:{given_name:'Новый',family_name:'Участник',birth_date:'2000-01-01',...(youth?{youth_id:youth,receipt_key:id(seq++)}:{})},role:'super_admin',guest_only:false,review_status:'verified'};
   await q('insert into auth.users(id,email,raw_user_meta_data,email_confirmed_at)values($1,$2,$3,$4)',[u,`${u}@example.test`,metadata,confirmed?new Date().toISOString():null]);
 };
 const confirm=async u=>{await db.exec('reset role');await q('update auth.users set email_confirmed_at=now() where id=$1',[u]);};
 const access=()=>val('select public.my_account_access()');
 const pendingRequest=async u=>{await db.exec('reset role');return val('select review_request_id from app_private.account_access where user_id=$1',[u]);};
 const decide=async(u,decision,by=sup)=>{const req=await pendingRequest(u);await login(by);await q('select public.review_youth_join_request($1,$2,$3)',[req,decision,by]);return req;};
 await check('unassigned registration waits safely; setting destination routes it without exposing clubs',async()=>{
   await signup(unassigned,{confirmed:true});await login(unassigned);
   assert.equal((await access()).restricted_guest,true);assert.equal((await access()).review_status,'pending');
   assert.equal((await q('select * from public.available_youth_clubs()')).rows.length,0);
   assert.equal((await q('select * from public.youth_groups')).rows.length,0);
   await denied('select public.registration_intake_settings()');
   await denied('select public.set_registration_intake($1,$2)',[a,unassigned]);
   await login(sup);assert.equal((await val('select public.registration_intake_settings()')).waiting_count,1);
   assert((await q("select * from public.notifications where kind='review_routing' and read_at is null")).rows.length>0);
   await q('select public.set_registration_intake($1,$2)',[a,sup]);
   assert.equal((await val('select public.registration_intake_settings()')).waiting_count,0);
   assert(await pendingRequest(unassigned));
 });
 await check('email confirmation creates exactly one review; metadata cannot grant access',async()=>{
   await signup(fresh);await login(fresh);
   assert.equal((await access()).review_status,'awaiting_email');assert.equal((await access()).restricted_guest,true);
   assert.equal((await q('select * from public.youth_club_directory()')).rows.length,0);
   assert.equal(await pendingRequest(fresh),null);
   await confirm(fresh);const req=await pendingRequest(fresh);assert(req);
   await confirm(fresh);assert.equal(await pendingRequest(fresh),req);
   await login(fresh);assert.equal((await access()).review_status,'pending');assert.equal((await access()).super_admin,false);
   for(const s of ['select public.club_entry($1)','select public.club_dashboard($1)'])await denied(s,[a]);
   await denied('select public.request_youth_club($1,$2,$3)',[b,id(seq++),fresh]);
   assert.equal(await val('select count(*)::int from public.chat_messages'),0);
   await denied('update app_private.account_access set review_guest=false where user_id=$1',[fresh]);
   await denied('select app_private.route_review($1)',[fresh]);
 });
 await check('notification identifies applicant and club; only entitled reviewers receive it',async()=>{
   const req=await pendingRequest(fresh);
   await login(sup);await roles(clubAdmin,[{id:a,role:'youth_admin'}]);
   // New application after restoring clubAdmin's existing scoped role.
   await signup(rejectUser,{confirmed:true});const req2=await pendingRequest(rejectUser);
   for(const u of [sup,leader,clubAdmin]){await login(u);const rows=(await q('select * from public.notifications where youth_request_id=$1',[req2])).rows;assert.equal(rows.length,1);assert.match(rows[0].body,/Новый Участник · Клуб А/);assert.equal(rows[0].target_path,`/youth-requests/${a}?request=${req2}`);}
   for(const u of [moderator,member,newcomer]){await login(u);assert.equal((await q('select * from public.notifications where youth_request_id=$1',[req2])).rows.length,0);await denied('select public.review_youth_join_request($1,$2,$3)',[req2,'accepted',u]);}
   await login(clubAdmin);await denied('select public.review_youth_join_request($1,$2,$3)',[req,'accepted',sup]);
   await denied('select public.youth_join_requests($1)',[b]);
 });
 await check('reject produces verified Guest; repeat is idempotent and opposing decision is rejected',async()=>{
   const req=await decide(rejectUser,'rejected',clubAdmin);
   await q('select public.review_youth_join_request($1,$2,$3)',[req,'rejected',clubAdmin]);
   await assert.rejects(()=>q('select public.review_youth_join_request($1,$2,$3)',[req,'accepted',clubAdmin]),e=>e.code==='40001');
   assert.equal(await val('select count(*)::int from public.notifications where youth_request_id=$1 and read_at is null',[req]),0);
   await login(rejectUser);assert.equal((await access()).review_status,'verified');assert.equal((await access()).restricted_guest,true);
   await denied('select public.club_entry($1)',[a]);assert.equal((await q('select * from public.youth_club_directory()')).rows.length,0);
   assert((await q("select body from public.notifications where kind='youth_decision'")).rows.some(n=>n.body.includes('Проверен.')));
   // A new session reads the same stored state.
   await login(null);await login(rejectUser);assert.equal((await access()).review_status,'verified');
 });
 await check('accept lifts only automatic Guest restriction and admits exactly the selected club',async()=>{
   await decide(fresh,'accepted',leader);await login(fresh);
   assert.equal((await access()).restricted_guest,false);assert.equal((await access()).review_status,'verified');
   assert.equal((await val('select public.club_entry($1)',[a])).can_open,true);assert.equal((await val('select public.club_entry($1)',[b])).can_open,false);
   await denied('select public.club_dashboard($1)',[b]);await dash();
   await confirm(fresh);await login(fresh);assert.equal((await access()).restricted_guest,false);
 });
 await check('explicit club destination is retained before confirmation, inbox is silent until confirmed',async()=>{
   await signup(explicit,{youth:b});const req=await pendingRequest(explicit);assert(req);
   assert.equal(await val('select count(*)::int from public.notifications where youth_request_id=$1',[req]),0);
   await confirm(explicit);await login(sup);assert.equal((await q('select target_path from public.notifications where youth_request_id=$1',[req])).rows[0].target_path,`/youth-requests/${b}?request=${req}`);
   await login(clubAdmin);await denied('select public.review_youth_join_request($1,$2,$3)',[req,'accepted',clubAdmin]);
   await decide(explicit,'accepted');await login(explicit);assert.equal((await val('select public.club_entry($1)',[b])).can_open,true);
 });
 await check('manual global Guest is preserved by admission; old member roles in another club remain intact',async()=>{
   await signup(restricted,{confirmed:true});await login(sup);await roles(restricted,[],'guest');
   await decide(restricted,'accepted');await login(restricted);assert.equal((await access()).restricted_guest,true);
   await login(member);const d=await dash(b);assert(d);assert.equal((await access()).review_status,null);
   await login(sup);assert.equal((await details(member)).clubs.find(c=>c.id===b).role,'youth_moderator');
 });
 await check('sole available club becomes the default; archived destinations do not receive new requests',async()=>{
   await db.exec('reset role');await db.exec('update app_private.registration_intake set youth_id=null');await q('update public.youth_groups set is_archived=true where id=$1',[b]);
   const single=id(9006);await signup(single,{confirmed:true});const req=await pendingRequest(single);assert.equal(await val('select youth_id from app_private.youth_join_requests where id=$1',[req]),a);
   await login(null);assert.equal((await q('select * from public.available_youth_clubs()')).rows.length,0);await denied('select * from public.youth_groups');
 });


 await check('review cannot be bypassed in role manager; explicit access change works after verification',async()=>{
   const u=id(9010);await signup(u,{confirmed:true});await login(sup);
   await assert.rejects(()=>roles(u,[],'user'),e=>e.message.includes('review_required'));
   await decide(u,'rejected');await login(sup);await roles(u,[],'user');await login(u);
   assert.equal((await access()).restricted_guest,false);assert.equal((await access()).review_status,'verified');
   assert.equal((await val('select public.club_entry($1)',[a])).can_open,false);
 });

 await check('previously unconfirmed registration enters review when email is confirmed',async()=>{
   await confirm(beforeConfirm);await login(beforeConfirm);
   assert.equal((await access()).restricted_guest,true);assert.equal((await access()).review_status,'pending');assert(await pendingRequest(beforeConfirm));
 });
 await checkNotificationPeople({db,root,login,check,sup,leader,clubAdmin,moderator,member,a,b});
 console.log(`${count} guest-review and existing-club checks passed.`);
